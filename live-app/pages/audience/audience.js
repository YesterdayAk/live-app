...
Page({
  ...
  onkeyboardHeightChange(e) {
    ...
    query.selectAll('.comment_list').boundingClientRect(function (rect) {
      that.setData({
        scrollTop: (rect[0].height * (1 / that.data.phoneHeight) / 2) + that.data.keyboardHeight / 2
      })
    }).exec();
  },
  //显示表情
  showEmoji() {
    this.setData({
      ...
      toolViewHeight: !this.data.emojiShow ? 300 * (1 / this.data.phoneHeight) : 0
    })
    this.pageUp()
  },
  //页面上推
  pageUp() {
    ...
    query.selectAll('.comment_list').boundingClientRect((rect) => {
      that.setData({
        // scrollTop: rect[0].height + that.data.toolViewHeight
  ...
  onReady: function () {
    ...
    let query = wx.createSelectorQuery();
    //获取顶部直播介绍的高度
    query.selectAll('.introduce').boundingClientRect((rect) => {
      that.setData({
        topDistance: rect[0].height * (1 / that.data.phoneHeight)
      })
    }).exec()
  },
